...
// wonDealAmounts: optional Map<unit_id, deal.amount> for actual sold prices
export function getInventoryStats(units, wonDealAmounts = null) {
  if (!Array.isArray(units)) {
    return { available: 0, reserved: 0, sold: 0, blocked: 0, under_maintenance: 0, total: 0, totalValue: 0, soldListedValue: 0, soldActualValue: 0 };
  }
  ...
  return units.reduce((acc, unit) => {
    ...
    acc.total += 1;
    const listed = Number(unit.total_price || unit.base_price || 0);
    acc.totalValue += listed;
    if (status === 'sold') {
      acc.soldListedValue += listed;
      const actual = wonDealAmounts ? Number(wonDealAmounts.get(unit.id) || listed) : listed;
      acc.soldActualValue += actual;
    }
    return acc;
  }, { available: 0, reserved: 0, sold: 0, blocked: 0, under_maintenance: 0, total: 0, totalValue: 0, soldListedValue: 0, soldActualValue: 0 });
}
